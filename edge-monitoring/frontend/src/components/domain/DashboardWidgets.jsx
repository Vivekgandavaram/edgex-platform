...
function Widget({ widget, onDelete, onResize, onDragStart, onDrop }) {
  return <GlassPanel draggable onDragStart={() => onDragStart(widget.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => onDrop(widget.id)} className="relative min-h-[220px] overflow-hidden p-4" style={{ gridColumn: `span ${widget.width || 4}`, gridRow: `span ${widget.height || 2}` }}>
...
export default function DashboardWidgets() {
  const [widgets, setWidgets] = useState(() => {
    try { return JSON.parse(localStorage.getItem('edgex_dashboard_widgets')) || [{ id: 'simulated-reading', title: 'Temperature probe', subtitle: 'Demo device / temperature', chart: 'Area', value: '24.8', unit: '°C', updated: 'just now', simulated: true, width: 4, height: 2 }]; } catch { return []; }
  });
  ...
  const add = (template) => { setWidgets((current) => [...current, { id: `${template.id}-${Date.now()}`, title: template.lens, subtitle: template.name, chart: template.chart, value: '24.8', unit: 'sample units', updated: 'simulated', simulated: true, width: 4, height: 2 }]); setCatalogOpen(false); };
